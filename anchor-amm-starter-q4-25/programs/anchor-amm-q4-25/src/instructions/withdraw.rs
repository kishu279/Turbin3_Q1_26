...
impl<'info> Withdraw<'info> {
    pub fn withdraw(
        ...
        let am = ConstantProduct::xy_withdraw_amounts_from_l(
            self.vault_x.amount,
            self.vault_y.amount,
            self.mint_lp.supply,
            amount,
        ...
        require!(am.x >= min_x && am.y >= min_y, AmmError::SlippageExceeded);

        self.withdraw_tokens(true, am.x)?;
        self.withdraw_tokens(false, am.y)?;
