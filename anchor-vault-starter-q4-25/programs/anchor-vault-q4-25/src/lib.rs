...
declare_id!("BhZoEPy8DvYAt6W5EJMy2WiryQ8EjBqSbNqKUhgS34Zq");
...
impl<'info> Withdraw<'info> {
    pub fn withdraw(&mut self, _amount: u64) -> Result<()> {
        ...
        // msg!("Vault amount: {}, Rent: {}", vault_amount, rent);
        // let amount = vault_amount.saturating_sub();

        let seed = self.vault_state.key();
        let bump = self.vault_state.vault_bump;
        let signer_seeds: &[&[&[u8]]] = &[&[b"vault", seed.as_ref(), &[bump]]];
...
impl<'info> Close<'info> {
    pub fn close(&mut self) -> Result<()> {
        msg!("Account data cleared by the anchor itself");
        let cpi_program = self.system_program.to_account_info();
        let cpi_accounts = Transfer {
            from: self.vault.to_account_info(),
            to: self.user.to_account_info(),
        };
        let seeds = self.vault_state.key();
        let bump = self.vault_state.vault_bump;
        let signer_seeds: &[&[&[u8]]] = &[&[b"vault", seeds.as_ref(), &[bump]]];

        let cpi_context = CpiContext::new_with_signer(cpi_program, cpi_accounts, signer_seeds);

        transfer(cpi_context, self.vault.to_account_info().lamports())
    }
}
